...
class ChatAdminController extends Controller
{
    public function index()
    {
        $sessions = DB::table('chat_sessions')
            ...
            ->get()->map(fn($s) => array_merge((array)$s, [
                'last_message' => DB::table('chat_messages')->where('session_id', $s->id)->latest()->value('message'),
                'unread_count' => DB::table('chat_messages')->where('session_id', $s->id)->where('sender_type', 'visitor')->where('is_read', false)->count(),
            ]));
    ...
    public function join(Request $request, int $id)
    {
        ...
        DB::table('chat_messages')->insert(['session_id' => $id, 'sender_type' => 'system', 'message' => '✅ ' . auth()->user()->name . ' has joined. You are now connected with a live agent!', 'message_type' => 'system', 'is_read' => false, 'created_at' => now(), 'updated_at' => now()]);
    ...
    public function close(int $id)
    {
        ...
        DB::table('chat_messages')->insert(['session_id' => $id, 'sender_type' => 'system', 'message' => 'Chat closed by agent. Thank you!', 'message_type' => 'system', 'is_read' => false, 'created_at' => now(), 'updated_at' => now()]);
        return response()->json(['ok' => true]);
    }

    public function heartbeat()
    {
        DB::table('users')->where('id', auth()->id())->update(['is_online' => true, 'last_seen_at' => now()]);
        return response()->json(['waiting' => DB::table('chat_sessions')->where('status', 'waiting')->count(), 'ok' => true]);
    }
